...
for digit in range(0,10):
    print "\nDigit: ",digit
    allPngFiles = testingDir#glob.glob(testingDir+"/"+str(digit)+"/*.png")
    ...
    def workOnPNG(pngFile):
        ...
        for i in range(0,len(pixels)):
            pixelValue = pixels[i]/255.0
            verifierProcess.stdin.write("Assert <= "+str(pixelValue-0.002)+" "+"1.0 inX"+str(i)+"\n")
            verifierProcess.stdin.write("Assert >= "+str(pixelValue+0.002)+" "+"1.0 inX"+str(i)+"\n")
